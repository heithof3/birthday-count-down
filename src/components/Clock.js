...
class Clock extends Component {
  constructor(props) {
    ...
    this.noBirthYear =
      new Date(this.birthday).getFullYear() == new Date().getFullYear();

    this.state = {
      timeRemaining: this.getTimeRemaining(
        props.birthdayFormState.startDate.toString()
      ),
    };
  }
  ...
  renderMessage = function () {
    if (this.noBirthYear) {
      return <h4>untill your Birthday</h4>;
    }
    return <h4>remaining untill you are {this.getAge()}</h4>;
  }.bind(this);
      ...
      <div className="countdown">
        {this.state.timeRemaining == 0 ? (
          <div className="message-container">
            <p className="message-container_title">Countdown Complete</p>
            <p className="message-container_message">HAPPY BIRTHDAY!</p>
          </div>
        ) : (
          <div>
            ...
            <div className="until-container">{this.renderMessage}</div>
          </div>
        )}
      </div>
    );
